...
export default function ExpensesPage() {
  ...
  const [categories, setCategories] = useState<
    Array<{ id: string; name: string; iconName: string }>
  >([]);
  ...
  const hydrateExpenses = (
    expenses: ExpenseDTO[],
    allCategories: Array<{ id: string; name: string; iconName: string }>,
  ): Expenses[] =>
  ...
  useEffect(() => {
    const loadExpenses = async () => {
      try {
        const [fetchedCategories, expenses] = await Promise.all([
          getCategories(),
          getExpenses(),
        ]);
        ...
        setData(hydrateExpenses(expenses, fetchedCategories));
      } catch (err) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setData(hydrateExpenses(expenses, categories));
      setEditingId(null);
  ...
  const onDelete = async (expense: Expenses) => {
    ...
    try {
      ...
      setData((prev) => prev.filter((item) => item.id !== expense.id));
    } catch (err) {
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center h-10 rounded-md border transition ${
                        isSelected
